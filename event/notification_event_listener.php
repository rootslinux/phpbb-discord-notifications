...
class notification_event_listener implements EventSubscriberInterface
{
	...
	public function handle_topic_delete_action($event)
	{
		...
		if ($this->notification_service->is_notification_forum_enabled($query_data['forum_id']) == false)
		{
			return;
		}
		...
		$this->notify_topic_deleted($delete_data);
	}
	...
	public function handle_lock_action($event)
	{
		...
		{
			case 'lock_post':
				$this->notify_post_locked($lock_data);
				break;
			case 'unlock_post':
				$this->notify_post_unlocked($lock_data);
				break;
			case 'lock':
