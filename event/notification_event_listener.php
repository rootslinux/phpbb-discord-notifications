...
// 		}
	...
	/**
	 * Handles events generated by changing the lock status on a topic.
	 ...
	 * - Topic unlocked
	 */
	public function handle_topic_lock_action($event)
	{
		dumpy('----- handle_topic_lock_action');
		dumpy($event['action']);
		dumpy($event['ids']);
		dumpy($event['data']);

		// Notification messages can get complicated if we have multiple topics that are locked/unlocked in a single transaction.
		...
		// User data is for the user that started the topic (or is currently the first poster)
		$lock_data['user_id'] = $event['data'][$id]['topic_first_poster_id'];
		$lock_data['user_name'] = $event['data'][$id]['topic_first_poster_name'];

		// If the forum the post was made in does not have notifications enabled or the topic is not visible, do nothing more
		$topic_visibile = $event['data'][$id]['topic_visibility'] == 1 ? true : false;
		if ($this->notification_service->is_notification_forum_enabled($lock_data['forum_id']) == false || $topic_visibile == false)
		{
			return;
		}
		...
		{
			$this->notify_topic_locked($lock_data);
		}
		else
		{
			$this->notify_topic_unlocked($lock_data);
		}
	}
